...
class BaoKim_payment{
	...
	var $code 		= 'baokim';
	var $setting 	= array(
							'business' 				=> 'mxhieu8319972@gmail.com',//tai khoan cua nguoi nhan tien
							'merchant_id' 			=> '33743',//id cua website khi dang ky ben baokim
							'secure_pass' 			=> 'd786e0ea9489e80a',//mat khau giao tiep
							'cost_constant'			=> 1700,
							'cost_percent'			=> 1,
						);
	...
	// var $url 		= 'http://kiemthu.baokim.vn/payment/order/version11';	// URL chay test
 ...
 */
	...
	function payment($tran_id, $amount, $return_url)
	{
		...
		$url = $this->createRequestUrl($tran_id, $this->setting['business'], $amount, '', '', $tran_info, $url['success'], $url['cancel'], $url['detail']);
		redirect($url);
	}
 ...
 */
	...
	public function createRequestUrl($order_id, $business, $total_amount, $shipping_fee, $tax_fee, $order_description, $url_success, $url_cancel, $url_detail)
	{
		...
		$params['checksum']=hash_hmac('SHA1',implode('',$params),$this->setting['secure_pass']);
